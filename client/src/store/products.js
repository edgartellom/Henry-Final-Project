...
const useStore = create((set, get) => ({
  ...
  searchProducts: (words) =>
    set((state) => ({
      listProducts: state.products.filter((e) => {
        return (
          e.brand.toLowerCase().includes(words.toLowerCase()) ||
          e.name.toLowerCase().includes(words.toLowerCase()) ||
          e.model.toLowerCase().includes(words.toLowerCase())
        );
      }),
    })),
  ...
  setBrandFilter: (brand) => set((state) => ({ ...state, brandFilter: brand })),
  fetchProducts: async () => {
    try {
